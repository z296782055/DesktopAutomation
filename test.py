...
# ... (MockUtils and __main__ block remain the same) ...
if __name__ == '__main__':
    # ... (Previous MockUtils and test cases) ...
    class MockUtils:
        _cue_word = "Initial cue word.\nNo image initially."
        _img_path = "test_cue_image.png" # Path for image
        _has_image = False # Control flag
        def get_cue_word(self): return self._cue_word
        def set_cue_word(self, value): self._cue_word = value; print(f"Set cue word to: {value}")
        def get_cue_img_url(self):
            if self._has_image:
                p = Path(self._img_path)
                if not p.exists():
                    try: bmp = wx.Bitmap(500, 1500); dc = wx.MemoryDC(bmp); dc.SetBackground(wx.Brush(wx.Colour(220, 255, 220))); dc.Clear(); dc.SetTextForeground(wx.BLACK); font = wx.Font(18, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL); dc.SetFont(font); dc.DrawText("Tall (1500px) Dummy Image", 50, 50); dc.DrawLine(0, 0, 500, 1500); dc.DrawLine(500, 0, 0, 1500); del dc; bmp.SaveFile(str(p), wx.BITMAP_TYPE_PNG); print(f"Created dummy image: {self._img_path}")
                    except Exception as e: print(f"Could not create dummy image: {e}")
                return str(p)
            else:
                p = Path(self._img_path)
                if p.exists():
                    try: os.remove(p); print(f"Removed dummy image for no-image test: {self._img_path}")
                    except OSError as e: print(f"Could not remove dummy image: {e}")
                return str(p)
        def save_image_state(self, has_image):
            self._has_image = has_image; print(f"Simulating save: has_image set to {has_image}")
            if has_image: self.get_cue_img_url()
            else: self.get_cue_img_url()
    import sys
    if 'util.utils' not in sys.modules: import types; sys.modules['util.utils'] = types.ModuleType('utils')
    util.utils = MockUtils()
    app = wx.App(False)
    # --- Test Case 1: Start with NO image ---
    print("--- Test Case 1: Start with NO image ---"); util.utils.save_image_state(False); dialog1 = CueWordDialog(None); print("Dialog should open with Image Pane collapsed (button visible)."); dialog1.ShowModal(); dialog1.Destroy()
    # --- Test Case 2: Start WITH image ---
    print("\n--- Test Case 2: Start WITH image ---"); util.utils.save_image_state(True); dialog2 = CueWordDialog(None); print("Dialog should open with Image Pane expanded and image scrollable."); dialog2.ShowModal(); dialog2.Destroy()
    # --- Test Case 3: Reopen after having an image (should still have image) ---
    print("\n--- Test Case 3: Reopen (should still have image) ---"); dialog3 = CueWordDialog(None); print("Dialog should open with Image Pane expanded and image scrollable."); dialog3.ShowModal(); print("Simulating clearing image and saving..."); util.utils.save_image_state(False); dialog3.Destroy()
    # --- Test Case 4: Reopen after clearing image ---
    print("\n--- Test Case 4: Reopen after clearing image ---"); dialog4 = CueWordDialog(None); print("Dialog should open with Image Pane collapsed (button visible)."); dialog4.ShowModal(); dialog4.Destroy()
    print("\nAll tests finished."); app.MainLoop()
