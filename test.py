...
# Mock utils and validators for standalone running
class MockUtils:
    _data = {
        "泵": "P3230Q_(P3230Q)",
        "柱温箱": "CT3230_(CT3230)",
        "检测器": "UV3230_(UV3230)",
        "自动进样器": "AS3230_(AS3230)",
        "default_file_name": "%Y%m%d%H%M%S",
        "image_save_path": "D:\\fk\\test" # Use double backslash or raw string
    }
    def get_dictionary(self, key, default=""):
        return self._data.get(key, default)
    def set_dictionary(self, key, value):
        print(f"[MockUtils] Setting {key} = {value}")
        self._data[key] = value
    def traverse_elements(self, parent, element_type):
        # Simplified mock - real implementation might be more complex
        children = []
        for child in parent.GetChildren():
            if isinstance(child, element_type):
                children.append(child)
            children.extend(self.traverse_elements(child, element_type))
        return children
...
NumberValidator = MockValidator
OsPathValidator = MockValidator
utils = MockUtils()
# --- End Mock ---
...
class ConfigDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self.default_file_name_combo_box = wx.ComboBox(main_panel, name="default_file_name") # parent=main_panel
    ...
    def load_data(self):
        """Load data from storage into controls."""
    ...
    def save_data(self):
        """Save data from controls to storage."""
        # Example using TransferDataFromWindow if validators were fully implemented
    ...
    def on_submit(self, event):
        if self.save_data():
            # Use EndModal for dialogs, indicates success
            self.EndModal(wx.ID_OK)
            # Show confirmation *after* the config dialog closes
            wx.CallAfter(wx.MessageBox, "保存成功！", "提示", wx.OK | wx.ICON_INFORMATION)
...
if __name__ == '__main__':
    ...
    parent_frame = wx.Frame(None, title="Main App")
    parent_frame.Show()

    dialog = ConfigDialog(parent_frame)
    result = dialog.ShowModal() # Show the dialog modally
    ...
    dialog.Destroy()
    parent_frame.Destroy() # Clean up dummy frame
    app.MainLoop()
